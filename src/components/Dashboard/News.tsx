...
const News = () => {
  ...
  const handleCoverUpload = async (e) => {
    ...
    try {
      ...
      const storageRef = ref(storage, `notizie/cover/${file.name}`);
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      const uploadPromises = files.map(async (file) => {
        // @ts-ignore
        const storageRef = ref(storage, `notizie/${file.name}`);
        // @ts-ignore
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (selectedNews?.id) {
        const docRef = doc(db, "notizie", selectedNews.id);
        await updateDoc(docRef, data);
      } else {
        // @ts-ignore
        data.createdAt = serverTimestamp();
        await addDoc(collection(db, "notizie"), data);
      }
  ...
  const deleteNews = async () => {
    ...
    try {
      ...
      await deleteDoc(doc(db, "notizie", newsToDelete.id));
    ...
    <>
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle>Lista Notizie</CardTitle>
          <Button
            onClick={() => {
              setSelectedNews(null);
              setIsViewMode(false);
        ...
        <DialogContent>
          ...
          <div className="py-4">
            ...
            <ul className="list-disc list-inside mt-2 text-sm text-gray-500">
              <li>La notizia "{newsToDelete?.title}"</li>
